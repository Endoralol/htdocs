...
switch($_REQUEST['category'])
{
	case 'account':
		...
		if(!Text::IsNull($_REQUEST['subcategory']))
		{
			...
			switch($_REQUEST['subcategory'])
			{
				...
				case 'restore':
                    $Smarty->assign('CSRFToken', Session::GenerateCSRFToken());
					$Smarty->assign('Page', Page::Info('restore', array('bodycss' => 'wow-template security eu', 'pagetitle' => $Smarty->GetConfigVars('Account_Login').' - ')));
					$Smarty->display('pages/account_restore');
				break;

                case 'restoreaccount':
                    if(Account::VerifyEmailRestore($_REQUEST['email'])) {
                        $Smarty->assign('CSRFToken', Session::GenerateCSRFToken());
    					$Smarty->assign('Page', Page::Info('restore', array('bodycss' => 'wow-template security eu', 'pagetitle' => $Smarty->GetConfigVars('Account_Login').' - ')));
    					$Smarty->display('pages/account_restore');
                    }   else{
                ...
                case 'management':
                    ...
                    if(Text::IsNull($_REQUEST['lastcategory']))
                    {
                        ...
                        if(isset($_REQUEST['accountName']))
                            ...
                            $AccountID = $UserGame['id'];
                        $Smarty->assign('Account', Account::GetAccountByID($AccountID));
                        if(Account::VerifyAccountAccess($AccountID))
                        {
                            switch($_REQUEST['lastcategory'])
                            {
                                ...
                                case 'claim-code-status':
                                        ...
                                        Manager::LoadExtension('Soap', $ClassConstructor);
                                        $ActivationStatus = Shop::CodeActivated($UserGame['id'], $_REQUEST['key']);
                                        if(!$ActivationStatus)
                                            ...
                                            else
                                            {
                                                ...
                                                {
                                                    Shop::ChangeActivationState($UserGame['id'], $_REQUEST['key']);
                                                    ...
                                                    switch($_REQUEST['service'])
                                                    {
                                                        ...
                                                        case 'PNC':
                                                            if(!isset($_REQUEST['servicecat']))
                                                            {
                                                                ...
                                                                $CharID = Account::GetIdsAccounts($User['username']);
                                                                $Smarty->assign('Service', $Service);
                                                                $Smarty->assign('Characters', Characters::GetCharacters($CharID['id']));
                                ...
                                case 'balance':
                                    if(Text::IsNull($_REQUEST['datatype']))
                                    {
                                        $Smarty->assign('Page', Page::Info('account_operations', array('bodycss' => '', 'pagetitle' => $Smarty->GetConfigVars('Account_Management'))));
                                        $Smarty->display('account/account_balance');
                                    }
                                    else
                                    {
                                        if($_REQUEST['datatype'] == 'balance-detail')
                                        {
                                            ...
                                            $Smarty->display('account/account_balance_detail');
                                        }
                                        else
                                            header('Location: /account/management/balance');
                                    }
                                break;
                    ...
                    else
                    {
                        if($_SESSION['generated_captcha'] == $_REQUEST['captchaInput'])
                        {
                            ...
                            else
                            {
                                ...
                                $Smarty->assign('CSRFToken', Session::GenerateCSRFToken());
                                $Smarty->assign('ActivateAccount', $Smarty->GetConfigVars('Account_Activate'));
					            $Smarty->assign('Page', Page::Info('login', array('bodycss' => 'login-template web wow', 'pagetitle' => $Smarty->GetConfigVars('Account_Login').' - ')));
					            $Smarty->display('pages/account_login');
                            }
                        }
                    ...
                    if(!Text::IsNull($_REQUEST['accountName']) && !Text::IsNull($_REQUEST['password']) && !Text::IsNull($_REQUEST['persistLogin']) && !Text::IsNull($_REQUEST['csrftoken']) && !Text::IsNull($_REQUEST['captchaInput']))
                    {
                        if($_SESSION['generated_captcha'] == $_REQUEST['captchaInput'])
                        {
                            if(Session::ValidateCSRFToken($_REQUEST['csrftoken']))
                            {
                                if(filter_var($_REQUEST['accountName'], FILTER_VALIDATE_EMAIL))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        unset($_SESSION['generated_captcha']);
                                        header('Location: /account/login&errorCode=5');
                                    }
                                }
                                else
                                {
                                    ...
                                    else
                                    {
                                        Session::UnsetKeys(array('loggedin', 'username', 'remember_me'));
                                        unset($_SESSION['generated_captcha']);
                                        header('Location: /account/login&errorCode=4');
                                    }
                                }
                            }
                            else
                            {
                                Session::UnsetKeys(array('loggedin', 'username', 'remember_me'));
                                unset($_SESSION['generated_captcha']);
                                header('Location: /account/login&errorCode=3');
                            }
                        }
                        else
                        {
                            unset($_SESSION['generated_captcha']);
                            header('Location: /account/login&errorCode=1');
                        }
                    }
                    else
                    {
                        unset($_SESSION['generated_captcha']);
                        Session::UnsetKeys(array('loggedin', 'username', 'remember_me'));
                        header('Location: /account/login&errorCode=2');
                    }
				break;
			}
		}   else    {
            ...
            else
            {
                switch($_REQUEST['subcategory'])
                {
                    case 'dashboard':
                        ...
                        $Smarty->assign('UserAccount', Account::GetAccountUser());
                        $Smarty->assign('UserAccountBan', Account::GetAccountUserBan());
                            ...
                            switch($_REQUEST['lastcategory']){
                                case 'add-articles':
                                    if(Text::IsNull($_REQUEST['datatype'])){
                                        ...
                                        $Smarty->display('admin/articles_additem');
                                    } else {
                                        switch($_REQUEST['datatype']){
                                            case 'process':
                                                News::AddNews($_REQUEST);
                                                header('Location: /admin/dashboard/');
                                            break;
                                            default:
                                                header('Location: /admin/shop/add-articles');
                                            break;
                                        }
                                    }
                                break;
                                ...
                                break;
                                case 'delete-articles-complete':
                                    Shop::DeleteItem($_REQUEST['itemid']);
                                    header('Location: /admin/dashboard');
                                break;
                                case 'edit-articles-complete':
                                    Shop::UpdateItem($_REQUEST);
                                    header('Location: /admin/shop/edit-articles');
                                break;
                            }
                        }
                    break;
                ...
                if(!Text::IsNull($_REQUEST['lastcategory']))
                {
                    ...
                    switch($_REQUEST['lastcategory'])
                    {
                        ...
                        case 'advanced':
                            ...
                            $Smarty->assign('Specializations', "");
                            $Smarty->assign('Inventory', Characters::GetGearForCharacter($CharacterData['guid']));
                            $Smarty->assign('Glyphs', "");
                            $Smarty->assign('ArenaRating', "");
                            $Smarty->assign('Professions', $Professions);
        ...
        else
        {
            switch($_REQUEST['subcategory'])
            {
                ...
                case 'forums':
                    ...
                    $Smarty->assign('Forums', Forums::GetPopularThreads());
                break;
                    ...
                    else
                    {
                        switch($_REQUEST['lastcategory'])
                        {
                            case 'load.json':
                                ...
                                $Smarty->assign('Comments', News::GetComments($ArticleID));
                                $Smarty->display('blog/comments_load');
                                break;
                        ...
                        else
                        {
                            if(Text::Match($_REQUEST['datatype'], 'post'))
                            {
                                if(Session::ValidateCSRFToken($_REQUEST['csrftoken']))
                                {
                                    ...
                                    $TopicID = Forums::CreateTopic($_REQUEST['subcategory'], $SelectedCharacterForComments['name'], $_REQUEST['subject'], $_REQUEST['postCommand_detail'], $GmAdd, $User['id']);
                        ...
                        switch($_REQUEST['datatype'])
                        {
                            case 'post':
                                if(Session::ValidateCSRFToken($_REQUEST['csrftoken']))
                                {
                                    ...
                                    $TopicData = Forums::GetTopicData($_REQUEST['lastcategory']);
                                    $TopicID = Forums::AddTopicComments($_REQUEST['lastcategory'], $TopicData['category']['id'], $SelectedCharacterForComments['name'], $_REQUEST['postCommand_detail'], $GmAdd, $User['id']);
                                    header('Location: /forum/topic/'.$TopicID);
                                }
                                else
                                    header('Location: ' . $_SERVER['HTTP_REFERER']);
                            ...
                            case 'open':
                            if($User['access_level'] >= 1) {
                                if(Forums::TopicOpen($_REQUEST['lastcategory'])) {
                                    header('Location: ' . $_SERVER['HTTP_REFERER']);
                                }
                            } else {
                                Page::GenerateErrorPage($Smarty, 404);
                            ...
                            case 'closed':
                            if($User['access_level'] >= 1) {
                                if(Forums::TopicClosed($_REQUEST['lastcategory'])) {
                                    header('Location: ' . $_SERVER['HTTP_REFERER']);
                                }
                            } else {
                                Page::GenerateErrorPage($Smarty, 404);
                            ...
                            case 'delete':
                            if($User['access_level'] >= 1) {
                                if(Forums::TopicDelete($_REQUEST['lastcategory'])) {
                            ...
                            case 'unsticky':
                            if($User['access_level'] >= 1) {
                                if(Forums::TopicUnsticky($_REQUEST['lastcategory'])) {
                                    header('Location: ' . $_SERVER['HTTP_REFERER']);
                                }
                            } else {
                                Page::GenerateErrorPage($Smarty, 404);
                            ...
                            case 'sticky':
                            if($User['access_level'] >= 1) {
                                if(Forums::TopicSticky($_REQUEST['lastcategory'])) {
                                    header('Location: ' . $_SERVER['HTTP_REFERER']);
                                }
                            } else {
                                Page::GenerateErrorPage($Smarty, 404);
                            ...
                            case 'deleteTopicUser':
                            if($User['access_level'] >= 1) {
                                if(Forums::TopicDeleteUser($_REQUEST['lastcategory'])) {
                                    header('Location: ' . $_SERVER['HTTP_REFERER']);
                                }
                            } else {
                                Page::GenerateErrorPage($Smarty, 404);
                            ...
                            case 'sendEdit':
                                Forums::EditTopicComments($_REQUEST['lastcategory'], $_REQUEST['idpost'], $_REQUEST['postCommand_detail']);
                                header('Location: /forum/topic/'.$_REQUEST['idtopic']);
                            break;

                            case 'edit':
                                $TopicData = Forums::GetPostsData($_REQUEST['lastcategory']);
                                $Smarty->assign('CSRFToken', Session::GenerateCSRFToken());
                                $Smarty->assign('TopicData', $TopicData);
                                $Smarty->assign('AccesUser', $User['access_level']);
                                $Smarty->assign('Page', Page::Info('forum', array('bodycss' => 'forums view-topic logged-in', 'pagetitle' => $TopicData['topic']['name'].' - ')));
                                $Smarty->display('pages/forums_edit_topic');
                            break;
                        }
                    }
                }
	...
	case 'bugtracker':
        Manager::LoadExtension('Bugtracker', $ClassConstructor);
		if(Text::IsNull($_REQUEST['subcategory']))
		{
            $BugTracker = Bugtracker::GetAllData();
            $Smarty->assign('BugTracker', $BugTracker);
			$Smarty->assign('Page', Page::Info('bugtracker', array('bodycss' => 'forums view-forum', 'pagetitle' => $Smarty->GetConfigVars('Menu_Community').' - ')));
			$Smarty->display('bugtracker');
		}
        else
        {
            switch($_REQUEST['subcategory'])
            {
                case 'add':
                    $Smarty->assign('Page', Page::Info('community', array('bodycss' => 'realm-status', 'pagetitle' => $Smarty->GetConfigVars('Menu_Community').' - ')));
                    $Smarty->display('pages/add_bugtracker');
                break;
