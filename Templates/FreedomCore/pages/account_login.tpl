<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="{$Language}" class="{$Language}">
	...
	<body class="{$Language} login-template web wow Cata" data-embedded-state="STATE_LOGIN">
			...
			<div class="input-container" id="login-wrapper">
				...
				"password-form" method="post" name="password-form">
                            ...
                            <div id="display-errors" class="alert alert-error alert-icon">
                                {if $ErrorCode == 1}
                                    {#Account_Login_Error_1#}
                                {elseif $ErrorCode == 2}
                                    {#Account_Login_Error_2#}
                                {elseif $ErrorCode == 3}
                                    {#Account_Login_Error_3#}
                                {elseif $ErrorCode == 4}
                                    {#Account_Login_Error_4#}
                                {elseif $ErrorCode == 5}
                                    {#Account_Login_Error_5#}
                                {/if}<br/>
                            </div>
                            {/if}
					<div class="control-group">
						<label class="control-label" for="accountName" id=
						"accountName-label">{#Login_Username#}</label>

						<div class="controls">
							<input class="input-block input-large" id="accountName"
							maxlength="320" name="accountName" placeholder="{#Login_Username#}"
