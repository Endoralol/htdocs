<table class="forum-topics" id="forum-topics" data-forum-id="{$Forum.forum_id}" style="font-size: 13px;">
    ...
    {foreach $Forum.topics as $Topic}
        <tr id="postRow{$Topic.id}" class="stickied-topic {if $Topic.closed}locked{/if}" data-topic-id="{$Topic.id}" itemscope="itemscope" itemprop="blogPost" itemtype="http://schema.org/BlogPosting">
            <td class="icon-cell">
                                    <span class="topic-icon-wrapper">
                                        <span class="topic-icon"></span>
                                        {if $Topic.forum_info.gm}<span class="employee-icon"></span>{/if}
                                    </span>
            </td>
            <td class="title-cell" data-tooltip="#topic-{$Topic.id}" data-tooltip-options="{ &quot;location&quot;: &quot;mouse&quot; }">
                ...
                {if $Topic.attached}<span class="status-text">{#Posts_Attached#}</span>{/if}
            ...
            {if $Topic.forum_info.gm}
            <span class="author-name blizzard-post" itemprop="author">{$Topic.postername}</span>
