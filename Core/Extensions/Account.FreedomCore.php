...
Class Account
{
    ...
    public static function GetUserGame($Username)
    {
        $Statement = Account::$AuthConnection->prepare('SELECT id FROM account WHERE username = :username');
    ...
    public static function VerifyAccountAccess($AccountID)
    {
        $Statement = Account::$AuthConnection->prepare('SELECT id FROM account WHERE id = :id');
     ...
     * @return string
     */
    public static function GetBalance($Username, $BalanceJson = true)
    {
    ...
    public static function CreateTMPAccount($Username, $Password, $Email, $Code)
    {
        ...
        if(!$TMPUsernameUsed && !$TMPEmailUsed && !$UsernameUsed && !$EmailUsed)
        {
            ...
            Account::Create($Username, $HashedPassword, $Email);
            Account::CreateGameAccount($Username, $GameHashedPassword, $Email, $RegistrationDate);
    ...
    private static function CheckForGameUsername($Username) {
        $Statement = Account::$AuthConnection->prepare('SELECT username FROM account WHERE username = :username');
        $Statement->bindParam(':username', $Username);
        $Statement->execute();
        $Result = $Statement->fetch(PDO::FETCH_ASSOC);
        if($Result['username'] == $Username)
    ...
    private static function CreateGameAccount($Username, $Password, $Email, $RegistrationDate)
    {
        ...
        $Statement = Account::$AuthConnection->prepare('INSERT INTO account (username, sha_pass_hash, email, joindate, last_login, expansion) VALUES (:username, :password, :email, :regdate, :lastlogin ,0)');
        $Statement->bindParam(':username', $Username);
        $Statement->bindParam(':password', $Password);
        $Statement->bindParam(':email', $Email);
        $Statement->bindParam(':regdate', $RegistrationDate);
        $Statement->bindParam(':lastlogin', $RegistrationDate);
        $Statement->execute();
        return 0;
    }
    ...
    public static function Authorize($Username, $Password)
    {
        ...
        if(!is_null($Result['username']))
        {
            ...
            {
                $CharIDS = Account::GetIdsAccounts($Result['username']);
                $CharID = Characters::PickRandomChar($CharIDS['id']);
                if($CharID != false)
                    Account::PinCharacter($Result['username'], $CharID);

                //$UserChar = Account::GetCharacters($CharID['id']);
                //$UserChars = Account::PinCharacter($Result['username'], $UserChar);
            }
    ...
    public static function GetIdsAccounts($Username)
    {
        $Email = strtoupper($Username);
        $Statement = Account::$AuthConnection->prepare('SELECT id FROM account WHERE username = :username');
        $Statement->bindParam(':username', $Username);
        $Statement->execute();
        $Result = $Statement->fetch(PDO::FETCH_ASSOC);
    ...
    {
        $Statement = Account::$DBConnection->prepare('SELECT COUNT(*) FROM users');
        $Statement->execute();
        $Result = $Statement->fetch(PDO::FETCH_COLUMN);
        return $Result;
    }

    public static function GetAccountUserBan()
    {
        $Statement = Account::$AuthConnection->prepare('SELECT COUNT(*) FROM `account_banned` WHERE `active` = 1');
        $Statement->execute();
        $Result = $Statement->fetch(PDO::FETCH_COLUMN);
        return $Result;
    }
    ...
    public static function VerifyEmailRestore($Email)
    {
        $Statement = Account::$DBConnection->prepare('SELECT email FROM users WHERE email = :selectedemail');
        $Statement->bindParam(':selectedemail', $Email);
        $Statement->execute();
