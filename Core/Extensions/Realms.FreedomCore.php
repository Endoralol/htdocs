...
Class Realms
{
    ...
    {
        $Statement = Realms::$AConnection->prepare('SELECT id, name, address, port, icon, realmflags, timezone, population, realmbuilds FROM realmlist');
        $Statement->execute();
    ...
    private static function GetUptimeForRealm($RealmID)
    {
        ...
        $Statement->execute();
        return Text::GetTimeDiff($Statement->fetch(PDO::FETCH_ASSOC)['start_time']);
    }
